...
NNCAM_EVENT_EXPO_START         = 0x4000              # exposure start
NNCAM_EVENT_EXPO_STOP          = 0x4001              # exposure stop
NNCAM_EVENT_TRIGGER_ALLOW      = 0x4002              # next trigger allow
NNCAM_EVENT_FACTORY            = 0x8001              # restore factory settings

NNCAM_OPTION_NOFRAME_TIMEOUT       = 0x01       # no frame timeout: 1 = enable; 0 = disable. default: disable
...
NNCAM_OPTION_MAX_PRECISE_FRAMERATE = 0x2c       # precise frame rate maximum value in 0.1 fps, such as 115 means 11.5 fps. E_NOTIMPL means not supported
...
NNCAM_OPTION_FRONTEND_DEQUE_LENGTH = 0x31       # frontend frame buffer deque length, range: [2, 1024], default: 3
NNCAM_OPTION_FRAME_DEQUE_LENGTH    = 0x31       # alias of NNCAM_OPTION_FRONTEND_DEQUE_LENGTH
NNCAM_OPTION_MIN_PRECISE_FRAMERATE = 0x32       # precise frame rate minimum value in 0.1 fps, such as 15 means 1.5 fps
...
NNCAM_OPTION_OPEN_USB_ERRORCODE    = 0x3b       # open usb error code
NNCAM_OPTION_LINUX_USB_ZEROCOPY    = 0x3c       # global option for linux platform:
...
NNCAM_OPTION_DUMP_CFG              = 0x3f       # explicitly dump configuration to ini, json, or EEPROM. when camera is closed, it will dump configuration automatically
NNCAM_OPTION_DEFECT_PIXEL          = 0x40       # Defect Pixel Correction: 0 => disable, 1 => enable; default: 1
NNCAM_OPTION_BACKEND_DEQUE_LENGTH  = 0x41       # backend frame buffer deque length (Only available in pull mode), range: [2, 1024], default: 3
...
NNCAM_TEMP_DEF                 = 6503     # temp, default
NNCAM_TEMP_MIN                 = 2000     # temp, minimum
NNCAM_TEMP_MAX                 = 15000    # temp, maximum
...
NNCAM_TEC_TARGET_MAX           = 300      # TEC target: 30.0 degrees Celsius
...
# get the version of this dll, which is: 50.19675.20211010
    @classmethod
    def Version(cls):
        cls.__initlib()
    ...
    def put_GlobalOption(cls, iOption, iValue):
        cls.__initlib()
        return cls.__lib.Nncam_Version(None, ctypes.c_uint(iOption), ctypes.c_int(iValue))

    @classmethod
    def get_GlobalOption(cls, iOption):
        cls.__initlib()
        x = ctypes.c_int(0)
        self.__lib.Nncam_get_Option(None, ctypes.c_uint(iOption), ctypes.byref(x))
        return x.value
...
# get the temperature of the sensor, in 0.1 degrees Celsius (32 means 3.2 degrees Celsius, -35 means -3.5 degree Celsius)
    def get_Temperature(self, nTemperature):
        x = ctypes.c_ushort(0)
        #x = ctypes.c_short(0) ## ERROR in Touptek provided wrapper, this is c_short (should be c_ushort)
