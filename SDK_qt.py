...
# callbacks come from toupnam.dll/so internal threads, so we use qt signal to post this event to the UI thread
    ...
    def eventCallback(nEvent, nPara, ctx):
        if nEvent == toupnam.TOUPNAM_EVENT_IMAGE:
            ctx.eventImage.emit()
...
# run in the UI thread
    ...
    def initCamera(self):
        ...
        self.setWindowTitle('wait 5 seconds to discover the online cameras')
        time.sleep(5)
        a = toupnam.toupnam.Enum()
        if len(a) <= 0:
            self.setWindowTitle('No camera found')
            self.cb.setEnabled(False)
        else:
            self.camname = a[0].model
            self.setWindowTitle(self.camname)
            self.eventImage.connect(self.eventImageSignal)
            try:
                self.hcam = toupnam.toupnam.Open(a[0].id)
            except toupnam.HRESULTException:
                ...
                self.cb.setChecked(self.hcam.get_Para(toupnam.TOUPNAM_PARA_AEXPO) != 0)
                try:
    ...
    def changeAutoExposure(self, state):
        if self.hcam is not None:
            self.hcam.put_Para(toupnam.TOUPNAM_PARA_AEXPO, 1 if (state == Qt.Checked) else 0)
